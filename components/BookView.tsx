  ...
  return (
    <div className="flex flex-col h-full bg-white sm:rounded-[2.5rem] overflow-hidden">
      ...
      <div className="sticky top-0 z-40 bg-white p-6 border-b border-gray-100 shadow-sm">
         <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-gray-50 hover:bg-gray-100 rounded-full text-gray-400 hover:text-gray-600 transition-colors z-50"><X size={20} /></button>
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={() => onTagClick(book.series)} className="col-span-2 bg-gray-50 p-3 rounded-2xl border border-gray-100 flex items-center gap-3 text-left hover:bg-indigo-50 hover:border-indigo-100 transition-all active:scale-95">
                ...
                <div className="min-w-0 flex-1 flex justify-between items-center">
                    <div><p className="text-[8px] text-gray-400 uppercase font-bold">Серія</p><p className="text-[10px] font-bold text-gray-700 truncate">{book.series || '—'}</p></div>
            ...
            {book.status === 'Completed' ? (
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-pink-50/50 p-4 rounded-3xl border border-pink-100 flex items-center gap-3">
                    ...
                    <div><p className="text-[10px] font-bold text-pink-400 uppercase">В бажаннях з</p><p className="text-xs font-bold text-pink-700">{new Date(book.addedAt).toLocaleDateString('uk-UA')}</p></div>
                </div>
            ) : (
            ...
            {(book.notes || book.comment) && (
                <div className="space-y-3 pt-2">
                    ...
                    {book.comment && (
                        <div className="bg-gray-50 p-4 rounded-3xl border border-gray-100">
                            <div className="flex items-center gap-2 mb-2"><MessageSquare size={14} className="text-gray-400" /><span className="text-[10px] font-bold text-gray-400 uppercase">Коментар</span></div>
            ...
            <div className="flex gap-2 mt-4 pb-8">
              <button onClick={onEdit} className="flex-1 bg-gray-100 text-gray-800 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95 transition-all"><Edit3 size={18} /> Редагувати</button>
